...
const CurrentPlan = ({ data }: { data?: PricingPlanType[] }) => {
  // Vars
  const buttonProps = (children: string, variant: ButtonProps['variant'], color: ThemeColor, overRideColor?: string): ButtonProps => ({
    children,
    variant,
    color,
      ...
      <CardContent>
        <Grid container spacing={6}>
          <Grid size={{ xs: 12, md: 6 }} className='flex flex-col gap-4'>
            <div>
              <Typography variant='h6'>Your Current Plan is Basic</Typography>
              <Typography>A simple start for everyone</Typography>
            </div>
            <div>
              <Typography variant='h6'>Active until Dec 09, 2021</Typography>
              <Typography>We will send you a notification upon Subscription expiration</Typography>
            </div>
            <div className='flex flex-col gap-1'>
              <div className='flex items-center gap-2'>
                <Typography variant='h6'>$99 Per Month</Typography>
                <Chip color='primary' label='Popular' size='small' variant='tonal' sx={{
                  "& .MuiChip-label": {
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            ...
            <LinearProgress variant='determinate' value={80} className='mlb-1 bs-2.5' sx={{
