...
const DebouncedInput = ({
    ...
    return <TextField {...props} value={value} onChange={e => setValue(e.target.value)} size='small' sx={{
            ...
            columnHelper.accessor('status', {
                ...
                cell: ({ row }) => (
                    <div className='flex items-center'>
                        <span
                            className={`px-2 py-1 rounded-full text-xs ${row.original.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                                }`}
                        >
                            {row.original.status}
                        </span>
